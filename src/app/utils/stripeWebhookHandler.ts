...
async function handlePaymentIntentSucceeded(paymentIntent: Stripe.PaymentIntent) {
  ...
  try {
    ...
    const donation = await DonationService.findDonationByPaymentIntentId(
      paymentIntent.id
    );
...
async function handlePaymentIntentFailed(paymentIntent: Stripe.PaymentIntent) {
  ...
  try {
    ...
    if (donation) {
      logger.info(
        `Donation with payment intent ${paymentIntentId} marked as failed`
      );
    } else {
...
async function handlePaymentIntentCanceled(paymentIntent: Stripe.PaymentIntent) {
  ...
  try {
    ...
    if (donation) {
      logger.info(
        `Donation with payment intent ${paymentIntentId} marked as canceled`
      );
    } else {
      logger.error(`No donation found for payment intent ${paymentIntentId}`);
    }
    ...
    logger.error(
      `Error handling payment intent cancellation: ${error.message}`
    );
    throw error;
  }
}
